function [output] = ft_windowERF(cfg,data)
% Determine time-domain window
% 
...
if ~isfield(cfg,'xtoi')
    ...
    error('Provide time labels!');
end

if ~isfield(cfg,'chan')
    cfg.chan = 'FCz'; 
    cfg.chanIndex = find(ismember(data.label,cfg.chan));
else
    cfg.chanIndex = find(ismember(data.label,cfg.chan));
    cfg.chan = {data.label{cfg.chanIndex}};
end

if isstruct(data) && ndims(data.avg) == 2
    dataMat = data.avg;
elseif isstruct(data) && ndims(data.avg) == 3 % multi-trial and channel data
    if ~isfield(cfg,'trials') cfg.trials = 1; end % default select only first trial
    dataMat = squeeze(data.avg(cfg.trials,:,:)); % freq_time matrix
else
...
dataMatWindow(~ismember(1:size(dataMat,1),cfg.chanIndex),:) = NaN;
% imagesc(dataMatWindow)

%% determine values in window 1 (initial window)

output = [];

output.window1.desc = 'x: time, y = chan';
